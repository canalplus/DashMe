...
func (b *DASHBuilder) buildLiveManifest() (string, error) {
	bufferDepth := math.MaxFloat64
	...
	for i := 0; i < len(b.tracks); i++ {
		...
		if b.tracks[i].BufferDepth() < bufferDepth {
			bufferDepth = b.tracks[i].BufferDepth()
		}
...
<MPD
  ...
  timeShiftBufferDepth="PT` + strconv.FormatFloat(bufferDepth, 'f', -1, 64) + `S"
  maxSegmentDuration="PT` + strconv.FormatFloat(maxChunkDuration, 'f', -1, 64) + `S"
  minBufferTime="PT` + strconv.FormatFloat(minBufferTime, 'f', -1, 64) + `S"
...
func (b *DASHBuilder) Build(inPath string, filename string, isLive bool) error {
	...
	/* Build manifest */
	if isLive {
		manifest, _ = b.buildLiveManifest()
	} else {
